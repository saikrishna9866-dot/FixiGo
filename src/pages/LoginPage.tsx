...
export const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const { error } = await safeSignInWithPassword({ email, password });
    setLoading(false);
    if (error) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      // Use the current origin + the intended destination
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 pt-20 px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-yellow-500 text-white py-2 rounded-lg font-bold hover:bg-yellow-600 transition-colors disabled:opacity-50" disabled={loading || googleLoading || !isSupabaseConfigured}>
        ...
        <button 
          ...
          className="mt-4 w-full flex items-center justify-center gap-3 bg-white border border-gray-300 text-gray-700 py-2 rounded-lg font-medium hover:bg-gray-50 transition-colors disabled:opacity-50"
        >
          {googleLoading ? (
